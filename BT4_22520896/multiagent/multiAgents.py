    ...
    def getAction(self, gameState):
        ...
        def expValue(state, agentIdx, depth):
            if agentIdx == state.getNumAgents():
                return maxValue(state, 0, depth + 1)
            value = 0
            numberOfActions = len(state.getLegalActions(agentIdx))
            for action in state.getLegalActions(agentIdx):
                succ = expValue(state.generateSuccessor(agentIdx, action), agentIdx + 1, depth)
                value += succ
            if numberOfActions != 0 :
                return value/numberOfActions
            else:
                return self.evaluationFunction(state)
...
def betterEvaluationFunction2(currentGameState):
    ...
    for ghost in newGhostStates:
        if ghost.scaredTimer != 0:
            nomalGhosts.append(ghost)
        else:
            scaredGhosts.append(ghost)

    nomalGhostPos = [ghost.getPosition() for ghost in nomalGhosts]
    if nomalGhostPos:
        closestNomalGhost = min([manhattanDistance(newPos, ghost) for ghost in nomalGhostPos])
        score -= 3.0 * (1 / closestNomalGhost)

    scaredGhostPos = [ghost.getPosition() for ghost in scaredGhosts]
    if scaredGhostPos:
        closestScaredGhost = min([manhattanDistance(newPos, ghost) for ghost in scaredGhostPos])
        score -= 3.0 * closestScaredGhost

    return score - 20.0 * numberOfCapsules - 4.0 * numberOfFoods